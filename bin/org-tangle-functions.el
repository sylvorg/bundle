                ...
                (let* ((curl-README (shell-command-to-string (format "curl -fsSL %s 2> /dev/null" lf))))
                    (unless (s-blank? curl-README)
                        (let* ((temp (make-temp-file (uuidgen-5 (uuidgen-4) (uuidgen-4)))))
                            ...
                            temp)))))))
(defvar settings-path nil)
(defvar titan-settings nil)
(defun get-README (&optional return-link setupfile lobfile) (interactive)
    (or (link-or-file settings-path return-link)
        ...
        (let* ((README (f-join "settings" "README.org"))
                ...
                ;; TODO: What is this again? Comment here as a note.
            ...
            (cond
              ...
              (t (link-or-file "https://raw.githubusercontent.com/sylvorg/settings/main/README.org" return-link t))))))
