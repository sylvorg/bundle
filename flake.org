...
No longer used; circular imports are much too confusing...
...
(replace-regexp-in-string "\n\\'"
                            ...
                            "nix eval --impure --expr 'with builtins; with (import ./.).pkgs.${currentSystem}; with lib; attrNames (filterAttrs (n: v: all (b: b == true) [ (! (elem n [ ])) (tryEval v).success (v ? patchRegistryDeps) ]) pkgs)'"))
#+end_src
...
~j.filters.has.list~ is an incomplete function; the general form of ~withPackages~ is ~(ppkgs: ...)~, filling in the ~attrs~ for ~j.filters.has.list~:
