...
projectName := $(subst ",,$(shell (find $(mkfileDir) -name '.#*.org*' -print | xargs rm &> /dev/null || :) && ($(pnCommand) || (org-tangle -f $(preFiles) && $(pnCommand)))))
ifndef projectName
$(error Sorry; unable to get the name of the project)
endif

typeCommand := nix eval --show-trace --impure --expr '(import $(realfileDir)).type'
type := $(subst ",,$(shell (find $(mkfileDir) -name '.#*.org*' -print | xargs rm &> /dev/null || :) && ($(typeCommand) || (org-tangle -f $(preFiles) && $(typeCommand)))))
ifndef type
